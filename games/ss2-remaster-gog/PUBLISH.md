# SS2 Hub publication review

Entry: [System Shock 2 Remaster, GOG](https://hub.splitscreen.me/handler/FFBhS68GK5T7u2CEP).

Source: [alex-nax/coop — SS2](https://github.com/alex-nax/coop/tree/main/games/ss2-remaster-gog). Made with AI assistance using OpenAI Codex, with human testing.

## Edit & Manage

| Field | Prepared value |
| --- | --- |
| Title | System Shock 2 Remaster (GOG, 2 gamepads) |
| Maximum players | 2 |
| Controller support | Enabled |
| Mouse/keyboard support | None |
| Description | Paste the descriptive paragraphs from [hub-description.md](hub-description.md), including GitHub links and AI disclosure |

The existing entry has its placeholder description and advertises mouse/keyboard support; the handler supports two gamepads. Correct both before submitting.

## First release

Choose **Release a new version** and select `~/coop/dist/ss2-remaster-gog-0.1.0.nc`. The Windows copy is in the mirror's `dist` folder. Use this short description:

```text
v0.1.0: GOG 1.3, two gamepads, top/bottom split, separate saves and confirmed LAN connection. AI-assisted (OpenAI Codex), human-tested. Source: https://github.com/alex-nax/coop
```

After the upload succeeds, enable **I want to publicly share my handler** and save. This control is disabled while no release exists. Public availability also requires Hub administrator authorization, so **Pending validation** is a possible result. Do not call the handler officially verified before that review occurs.

Review the resulting SS2 page and release first. NWN2 EE publication waits for the owner's review, as requested.
